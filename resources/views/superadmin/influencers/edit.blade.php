...
@section('title', __('Edit Influencer'))
@section('breadcrumb')
    <li class="breadcrumb-item"><a href="{{ route('home') }}">{{ __('Dashboard') }}</a></li>
    <li class="breadcrumb-item"><a href="{{ route('users.influencers', $user->tenant_id) }}">{{ __('Influencers') }}</a></li>
    <li class="breadcrumb-item">{{ __('Edit Influencers') }}</li>
@endsection
@section('content')
        ...
        <section class="section">
            <div class="m-auto col-lg-6 col-md-8 col-xxl-4">
                <div class="card">
                    ...
                    <div class="card-body">
                        {!! Form::model($user, [
                            'route' => ['superadmin.influencer.update', ['tenant_id' => $user->tenant_id, 'influencer_id' => $user->id]],
                            'method' => 'Post',
                            'data-validate',
                        ]) !!}
                            ...
                            <select class="form-control form-control-inline-block" data-trigger name="subscription_plan_id"
                                ...
                                @foreach ($subscriptions as $subscription)
                                    <option value="{{ $subscription->id }}"
                                        {{ $user->subscription_plan_id == $subscription->id ? 'selected' : '' }}>
                    ...
                    <div class="card-footer">
                        <div class="float-end">
                            <a href="{{ route('users.influencers', $user->tenant_id) }}" class="btn btn-secondary">{{ __('Cancel') }}</a>
