                        ...
                        <div class="focus:outline-none mt-3 mb-3 lg:mt-24">
                            <div class="infinity">
                                <div class="flex flex-wrap w-100">
                                    {{-- Always display Album Category as first box --}}
                                    @if($album_category)
                                        <div class="focus:outline-none w-full md:w-1/2 lg:w-1/3 py-3 p-sm-3 max-w-md">
                                            <div class="shadow rounded-2 overflow-hidden position-relative">
                                                ...
                                                @if ($album_category->file_type == 'image' || is_null($album_category->file_type))
                                                    <img class="w-full post-thumbnail open-full-thumbnail"
                                                        src="{{ asset($album_category->image) }}" alt="{{ $album_category->name }}" />
                                                @else
                                                    <video controls class="w-full post-thumbnail">
                                                        <source src="{{ asset($album_category->image) }}" type="video/mp4">
                                                    </video>
                                                @endif

                                                <div class="px-4 py-2">
                                                    <div class="text-md italic text-gray-500">
                                                        {{ \Carbon\Carbon::parse($album_category->created_at)->format('d M Y') }}
                                                    </div>
                                                    <h1 class="text-xl font-bold truncate">
                                                        {{ $album_category->name }}
                                                    </h1>

                                                    @if (!empty($album_category->description))
                                                        <div class="hidden long-text text-gray-600"
                                                            style="font-size: 15px; max-height: 100px; overflow-y: auto;">
                                                            {!! $album_category->description !!}
                                                        </div>
                                                        <a href="javascript:void(0)"
                                                            data-long_description="{{ e($album_category->description) }}"
                                    ...
                                    @if ($albums->count() > 0)
                                        ...
                                        @endforeach
                                    @else
                                        {{-- Show message only when there are no albums (album category is already shown above) --}}
                                        @if($album_category)
                                            <p class="text-gray-500 text-lg mt-5">No albums available in this category.</p>
                                        @else
