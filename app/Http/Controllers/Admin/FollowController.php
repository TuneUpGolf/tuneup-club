...
class FollowController extends Controller
{
    public function followInstructorApi(Request $request)
    {
        ...
        try {
            ...
            if (Follow::where('follower_id', Auth::user()->id)->where('isPaid', Follow::FOLLOW)->where('instructor_id', $instructorId)->exists()) {
                return response()->json(['message' => 'Student already follows this instructor'], 422);
            }

            if (Follow::where('follower_id', Auth::user()->id)->where('isPaid', Follow::SUBSCRIPTION)->where('instructor_id', $instructorId)->exists()) {
    ...
    public function unfollowInstructor(Request $request)
    {
        ...
        try {
            ...
            Follow::where('follower_id', $studentId)->where('instructor_id', $instructorId)->delete();
    ...
    public function subscribeInstructor(Request $request)
    {
        ...
        try {
            if (Follow::where('follower_id', Auth::user()->id)->where('isPaid', Follow::FOLLOW)->where('instructor_id', $instructorId)->exists()) {
                $follow = Follow::where('follower_id', Auth::user()->id)->where('instructor_id', $instructorId)->first();
    ...
    public function getStudents()
    {
        try {
            if (Auth::user()->type === Role::ROLE_INSTRUCTOR && Auth::user()->active_status == 1) {
                return response()->json(Follow::where('instructor_id', Auth::user()->id)->with('follower')->paginate(request()->get('perPage')));
            } else {
