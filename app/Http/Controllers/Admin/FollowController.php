...
class FollowController extends Controller
{
    ...
    public function unfollowInstructor(Request $request)
    {
        ...
        try {
            ...
            Follow::where('student_id', $studentId)->where('influencer_id', $instructorId)->delete();
    ...
    public function subscribeInstructor(Request $request)
    {
        ...
        try {
            if (Follow::where('student_id', Auth::user()->id)->where('isPaid', Follow::FOLLOW)->where('influencer_id', $instructorId)->exists()) {
                $follow           = Follow::where('student_id', Auth::user()->id)->where('influencer_id', $instructorId)->first();
    ...
    public function getStudents()
    {
        try {
            if (Auth::user()->type === Role::ROLE_INFLUENCER && Auth::user()->active_status == 1) {
                return response()->json(Follow::where('influencer_id', Auth::user()->id)->with('student')->paginate(request()->get('perPage')));
            } else {
