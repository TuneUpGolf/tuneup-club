document.addEventListener("DOMContentLoaded", async () => {
  const { senderId, senderImage, groupId, recieverImage, token } =
    window.chatConfig;
  ...
  async function loadMessages(page = 1) {
    const response = await fetch(
      `${window.chatBaseUrl}/brainvire-chat-base-app/api/v1/chat/list`,
      {
        method: "POST",
        headers: {
  ...
  async function uploadChatMedia({ groupId, senderId, file }) {
    ...
    try {
      const response = await fetch(
        `${window.chatBaseUrl}/brainvire-chat-base-app/api/v1/chat/image`,
        {
          method: "POST",
          headers: {
            Authorization: `Bearer ${token}`,
